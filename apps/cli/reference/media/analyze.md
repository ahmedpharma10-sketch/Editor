...
- `--keep-video`: include the video stream when analyzing a video asset (optional). By default only the audio track is analyzed, which is significantly faster since the video stream is neither transcoded nor uploaded. No effect on audio or image assets.
