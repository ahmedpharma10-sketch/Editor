...
import type { SequenceAsset } from '../db';
...
export class SequenceDecoder {
	...
	private async initialize() {
		try {
			...
			this.frames = await Promise.all(entries.map(e => e.handle.getFile()));
...
export function resolveSequenceDecoder(world: EngineWorld, fid: number, hasCache = true): SequenceDecoder | null {
	...
	const existing = world.components.SequenceDecoder[fid];
	...
	addComponent(world, fid, world.components.SequenceDecoder, false);

	const decoder = new SequenceDecoder(asset, hasCache);
	world.components.SequenceDecoder[fid] = decoder;
	return decoder;
}
