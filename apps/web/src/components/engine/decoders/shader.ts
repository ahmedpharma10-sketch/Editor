 ...
 * paint carries code.
 */
export function resolveShaderHost(world: EngineWorld, fid: number): ShaderHost | null {
	const c = world.components;
	const code = c.Shader.code[fid];
	if (!code) return null;

	let host = c.ShaderHost[fid];
	if (!host) {
		addComponent(world, fid, c.ShaderHost, false);
		host = new ShaderHost();
		c.ShaderHost[fid] = host;
	}
	host.setCode(code);
	return host;
}
