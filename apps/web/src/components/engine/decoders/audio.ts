...
export function resolveAudioDecoder(world: EngineWorld, eid: number): ResolvedAudioDecoder | null {
	const AssetId = world.components.AssetId;
	const assetId = AssetId[eid];
	if (!assetId) return null;

	const existing = world.components.AudioDecoder[eid];
	...
	addComponent(world, eid, world.components.AudioDecoder, false);

	const decoder = new AudioDecoder(asset);
	world.components.AudioDecoder[eid] = decoder;
	return {
		decoder,
