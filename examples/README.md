...
Each root carries a stable `scene` identity, so re-mounting an example replaces its scene in place.

| Example | Shows |
| --- | --- |
| [01-basics.tsx](01-basics.tsx) | the `scene` prop promoting a root, `<sequence>` with a dissolve, `<video>`, `<audio>`, `<image>`, `<text>` titles from data via `<For>` |
...
| [04-html-in-canvas.tsx](04-html-in-canvas.tsx) | `<html>`: an AI prompt box as real DOM, typed out from the playhead |
