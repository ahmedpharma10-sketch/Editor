...
- **Asset id**: e.g. `"gbHJ"`, an imported asset (discover ids with `dapi asset ls` or `dapi asset tree`).
...
In `dapi node patch`, `src` takes a path, URL, or asset id; `generate.*` declarations are JSX-only.
