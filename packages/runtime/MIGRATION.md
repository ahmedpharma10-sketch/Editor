...
| Panels: header, background, scene template, asset info, time, appearance, alignment, export, layout, text, strokes, shadows, effects, animations, transition, masks, audio | `components/sidebar-right/inspector/` |
...
Still on bitecs: transform, caption-settings, fills, interpolation panels; the timeline's keyframe layer; the whole timeline UI.
...
**Known bug, not this panel's**: the `gain` animation writes a 0-1 linear ramp into `Computed.volume` (`motion.ts`, `AnimationType.GAIN`), which everything else reads as decibels (`audio-bus.ts` does `10 ** (db / 20)`). So a Volume animation ramps 0 dB to 1 dB, which is inaudible, instead of silence to unity. Carried over from bitecs; the fix is in the motion system, not in the panel.
