...
| Panels: header, background, scene template, asset info, time, appearance, alignment, export, layout, text, strokes, shadows, effects, animations, transition, masks, audio, fills, transform, caption | `components/sidebar-right/inspector/` |
...
Still on bitecs: the interpolation panel; the timeline's keyframe layer; the whole timeline UI. Auto-captions (`components/genai/use-auto-captions.ts`) is still bitecs too, so nothing in the app *creates* a caption on the koota side yet: a `<captions>` element reaches the world by being written in a project file.
