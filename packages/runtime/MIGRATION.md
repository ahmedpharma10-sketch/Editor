...
| Panels: header, background, scene template, asset info, time, appearance, alignment, export, layout, text, strokes, shadows, effects, animations | `components/sidebar-right/inspector/` |
...
Still on bitecs: transform, caption-settings, fills, transition, masks, audio, interpolation panels; the timeline's keyframe layer; the whole timeline UI.
