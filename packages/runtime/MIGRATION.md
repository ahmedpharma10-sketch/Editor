...
- `clampTrimToAssetDuration` now goes through `entity.set(Trim)` (fires change events) but only when actually clamping; bitecs wrote the store silently. `reactToChildDetached` is exported for the observers slice to hook Deleted onAdd.
...
1. **Observers / cache upkeep**: the bitecs `api/observers.ts` logic that fills `Cache` (children, fills, shadows, strokes, effects, textRanges, masks, keyframeTracks, keyframes, animations) from ChildOf + tags. Decide: koota `onAdd/onRemove(ChildOf('*'))` subscriptions (runtime-owned) vs recompute passes. Systems and timeline-index already consume `Cache`; actions rebuild it imperatively via `actions/cache.ts` (as bitecs did), and `reactToChildDetached` awaits its Deleted onAdd hook. Also decide where Deleted-tombstone pruning and `unpersistEntity` land.
2. **Media** (`src/media/`): move `engine/decoders/*` wholesale (video, audio, image, sequence, caption/, html, shader, surface, frame-cache, keyframe-index, time-stretcher, audio-peaks) plus `services/audio-bus.ts`. Then tighten the handle trait types in `src/traits/media.ts` to the concrete classes and delete the `Handle = object` placeholder. Reinstate the dispose hooks the actions slice dropped: `disconnectAudioBus` in appendChild/removeChild, decoder disposal in `reactToAssetChange`'s Caption branch, and the dispose* subtree walkers from `api/utils.ts`.
3. **Playback system**: `systems/playback.ts` (computes `Computed.localTime/start/end/duration/visibility`, drives decoders, pushes into `FramePromises`, uses AudioEngine + Mounts). Depends on media.
4. **Capture** (`src/capture/`): `engine/encode/*` (encoder, image-encoder, buffer, format, utils). Uses Mode, FramePromises, RenderSurface.
5. **Render system**: `systems/render.ts` is runtime-capable (OffscreenCanvas) and needed for headless capture; it consumes text layout (`utils/text.ts` full port: layout/measure/render join the Token seed).
6. **Fonts**: `engine/font/{utils,fixtures}.ts` (check DOM usage: document.fonts needs an injection seam or stays app-side).
7. **Remaining utils**: captions, transition, contact-sheet, audio-sync, transcode (audit each for DOM/app deps; DOM-touching ones stay app-side).
8. **App rewrite** (`apps/web`): engine shell wires canvas/AudioContext/DPR into world traits, app-side world traits (input, HUD, selection tool, history, store, timelineIndex signal), history/persistence via koota events (including wrapping the folder/asset model ops with Dexie writes and re-implementing `syncInteractiveState` over `ChildOf(document)` queries: `Not(ChildOf('*'))` throws in koota), UI to `packages/koota-solid` hooks, timeline UI consumes new `TimelineNode.entity`. Delete `engine/components`, `engine/api` progressively.
9. **CLI**: point node capture at the runtime package.
...
- Deps: koota (patched), mediabunny, animejs, sqids (asset/folder id allocation).
